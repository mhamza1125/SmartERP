...
class DeliveryController extends Controller
{
    ...
    public function index()
    {
        ...
        // Load related orders for multi-order deliveries
        $relatedOrdersMap = [];
        foreach ($delivery as $item) {
            if (isset($item->is_multi_order) && $item->is_multi_order) {
                $relatedOrders = $this->getRelatedOrdersForDelivery($item);
                if (!empty($relatedOrders)) {
                    $item->relatedOrders = $relatedOrders;
                    $relatedOrdersMap[$item->delivery_id] = $relatedOrders;
                }
            }
        }
        ...
        return view('delivery', [
            'delivery' => $delivery,
            'customers' => $customers,
            'relatedOrdersMap' => $relatedOrdersMap,
        ]);
    }
    ...
    public function store(DeliveryRequest $request)
    {
        ...
        // Auto-generate stock_no: use pipe-separated order IDs or temporary value
        // This is auto-generated by the system, never from user input
        if ($orderIds) {
            $validatedData['stock_no'] = str_replace(',', '|', $orderIds);
        } elseif (!empty($selectedOrders)) {
            $validatedData['stock_no'] = implode('|', $selectedOrders);
        } else {
            // For single-order deliveries: set temporary value, will update to delivery_id after creation
        ...
        // Update stock_no with delivery_id for single-order deliveries
        if (!$orderIds && empty($selectedOrders)) {
    ...
    public function update(Request $request, $id)
    {
        ...
        // Update stock_no with delivery_id only for single deliveries
        $orderIds = $request->input('order_ids');
        if ($orderIds) {
            $this->stockRepository->update($request->input('stock_id'), ['stock_no' => str_replace(',', '|', $orderIds)]);
        } elseif (!$isMultiOrder) {
        ...
        $this->storeDB(
            ...
            $request->input('rowQty'),
            $request->input('totalQty')
        );
