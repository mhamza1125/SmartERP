...
class IGroupRepository implements GlobalInterface
{
    public function all()
    {
        return IGroup::leftJoin('orders', 'orders.order_id', 'igroups.order_id')
    ...
    {
        return IGroup::leftJoin('orders', 'orders.order_id', 'igroups.order_id')
            ->orderBy('igroups.created_at', 'desc')
            ->where('igroup_status', '1')->get();
    }

    public function get($id)
    {
        return IGroup::where('igroup_id', $id)
            ->leftJoin('orders', 'orders.order_id', 'igroups.order_id')
