    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            <form action="{{ route('machine.store') }}" method="POST" class="needs-validation" novalidate="" enctype="multipart/form-data">
              @csrf
              <div class="row">
                <div class="col-md-2">
                  <div class="form-group">
                    <label>Machine No</label>
                    <input type="text" class="form-control" name="machine_no" required value="{{$refNo}}" readonly>
                    ...
                    <select class="form-control select2" name="machine_type_id" required>
                      ...
                      @if($head->count())
                        @foreach($head as $item)
                          <option value="{{$item->head_id}}" {{ old('machine_type_id') == $item->head_id ? 'selected' : '' }}>{{$item->name}}</option>
                        @endforeach
                      @endif
                    </select>
                    <div class="valid-feedback">Good job!</div>
                    ...
                    <select class="form-control select2" name="employee_id" required>
                      ...
                      @if($employee->count())
                        @foreach($employee as $item)
                          <option value="{{$item->employee_id}}" {{ old('employee_id') == $item->employee_id ? 'selected' : '' }}>{{$item->employee_no}} - {{$item->name}}</option>
                        @endforeach
                      @endif
                    </select>
                    <div class="valid-feedback">Good job!</div>
