@extends('index')
@section('content')
<section class="section">
  <div class="section-body">
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <h4>General Voucher Info</h4>
            <div class="card-header-action">
              <div class="btn-group">
                <a class="btn btn-info" href="{{ route('payment.print', $transaction['transaction_id']) }}" target="_blank">
                  <i class="fas fa-file-alt"></i> Print
                </a>
                <a href="{{ route('generalVoucher') }}" class="btn btn-primary">Back</a>
                <a href="{{ route('transaction.editGeneralVoucher', $transaction['transaction_id']) }}" class="btn btn-primary">Edit</a>
              </div>
            </div>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="col-md-7">
                <table class="table table-sm">
                  <tbody>
                    <tr><td><b>Voucher No:</b> SLE-{{ date('Y') }}-{{ str_pad($transaction['transaction_id'], 4, '0', STR_PAD_LEFT) }}</td></tr>
                    @if($transaction['bank_id'])
                      <tr><td><b>Paid By:</b> {{$transaction['bname']}}</td></tr>
                      <tr><td><b>Account Title:</b> {{$transaction['account_title']}}</td></tr>
                      <tr><td><b>Account No:</b> {{$transaction['account']}}</td></tr>
                    @else
                      <tr><td><b>Paid By:</b> Cash Payment</td></tr>
                    @endif
                    @php
                      $amount = 0;
                      // For customer general vouchers, use cc_amount (customer currency)
                      if ($transaction['transaction_to'] == 'customer' && isset($transaction['cc_amount']) && $transaction['cc_amount'] > 0) {
                          $amount = $transaction['cc_amount'];
                      } elseif (isset($transaction['debit']) && $transaction['debit'] > 0) {
                          $amount = $transaction['debit'];
                      } elseif (isset($transaction['credit']) && $transaction['credit'] > 0) {
                          $amount = $transaction['credit'];
                      }
                    @endphp
                    <tr><td><b>Amount:</b> {{ number_format($amount) }}</td></tr>
                    <tr><td><b>Amount in Words:</b> {{ numberToWordsWithCurrency($amount) }}</td></tr>
                    @if($transaction['description'])<tr><td><b>Detail:</b></td></tr>
                    <tr><td colspan="3">@php echo $transaction['description'] @endphp</td></tr>@endif
                  </tbody>
                </table>
              </div>
              <div class="col-md-5">
                <table class="table table-sm">
                  <tbody>
                    <tr><td><b>Payee Type:</b> 
                      @if($transaction['transaction_to'] == 'vendor')
                        Vendor
                      @elseif($transaction['transaction_to'] == 'contractor')
                        Contractor
                      @elseif($transaction['transaction_to'] == 'employee')
                        Employee
                      @elseif($transaction['transaction_to'] == 'customer')
                        Customer
                      @else
                        {{ ucfirst($transaction['transaction_to']) }}
                      @endif
                    </td></tr>
                    <tr><td><b>Payee Name:</b> 
                      @if($transaction['transaction_to'] == 'vendor' && $transaction['vendor_no'])
                        {{$transaction['vendor_no']}} - {{$transaction['vendor_name'] ?? $transaction['fname']}}
                      @elseif($transaction['transaction_to'] == 'contractor' && $transaction['vendor_no'])
                        {{$transaction['vendor_no']}} - {{$transaction['vendor_name'] ?? $transaction['fname']}}
                      @elseif($transaction['transaction_to'] == 'employee' && $transaction['employee_no'])
                        {{$transaction['employee_no']}} - {{$transaction['employee_name'] ?? $transaction['name']}}
                      @elseif($transaction['transaction_to'] == 'customer' && $transaction['customer_no'])
                        {{$transaction['customer_no']}} - {{$transaction['customer_name'] ?? $transaction['fname']}}
                      @else
                        N/A
                      @endif
                    </td></tr>
                    <tr><td><b>Voucher Date:</b> {{\Carbon\Carbon::parse($transaction['transaction_date'])->format('d-m-Y')}}</td></tr>
                  </tbody>
                </table>
              </div>
            </div>
            <div class="row">
              <div class="col-md-12">
                @if($image->count())
                  <h5>Images</h5>
                  <div id="aniimated-thumbnials" class="list-unstyled row clearfix">
                    @foreach($image as $item)
                      <div class="col-lg-3 col-md-4 col-sm-6 col-xs-12">
                        <a href="{{ URL::asset('resources/transaction/'. $item->image) }}">
                          <img class="img-responsive thumbnail" src="{{ URL::asset('resources/transaction/'. $item->image) }}" alt="">
                        </a>
                        <form action="{{route('image.delete', ['id' => $item->image_id, 'dir' => 'transaction'])}}" method="POST">
                          @csrf
                          <button type="submit" class="btn btn-danger delbtn"><i class="fa fa-trash"></i></button>
                        </form>
                      </div>
                    @endforeach
                  </div>
                @else
                  <blockquote> No Images </blockquote>
                @endif
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
</section>
@endsection
