                    ...
                    <select class="form-control select2" name="material_id[]" id="material_id">
                      ...
                      @if($issueItem->count())
                          @foreach($issueItem as $item)
                              <option value="{{$item->material_id}}|{{$item->product_type_id}}" {{ old('material_id') == $item->material_id ? 'selected' : '' }}>{{$item->name}} - {{$item->article_no}} - Size {{$item->sname}}</option>
                          @endforeach
                      @endif
                    </select>
                  </div>
                    ...
                    <select class="form-control select2" name="stage_id[]" id="stage_id" required>
                      @if($head->count())
                        @foreach($head as $item)
                          <option value="{{$item->head_id}}" {{ $loop->index == 0 ? 'disabled selected' : '' }} {{ old('head_id') == $item->head_id ? 'selected' : '' }}>{{$item->name}}</option>
                        @endforeach
                      @endif
                    </select>
                    <div class="valid-feedback">Good job!</div>
...
<script>
  $(document).ready(function() {
      ...
      $('#addBtnMaterial').click(function() {
          ...
          var markup = `<tr>
              <td>${srNo}</td>
              <td>${materialText}<input type="text" name="material_id[]" value="${materialId}"><input type="text" name="product_type_id[]" value="${productId}"></td>
              <td>${stageName}<input type="text" name="stage_id[]" value="${stageId}"></td>
              <td>${quantity}<input type="text" name="quantity[]" value="${quantity}"></td>
              <td><button type="button" class="btn btn-danger deleteRow">X</button></td>
          </tr>`;
