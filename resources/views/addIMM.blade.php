    ...
    <div class="row">
      <div class="col-12">
        <div class="card">
          ...
          <div class="card-body">
            <form action="{{ route('stock.store') }}" method="POST" class="needs-validation" novalidate="" enctype="multipart/form-data">
              @csrf
              <div class="row">
                    ...
                    <select class="form-control select2" name="machine_id" required>
                      ...
                      @if($machine->count())
                        @foreach($machine as $item)
                          <option value="{{$item->machine_id}}" {{ old('machine_id') == $item->machine_id ? 'selected' : '' }}>{{$item->machine_no}} | {{$item->name ? $item->employee_no .' - '. $item->name : 'Not Asigned'}}</option>
                        @endforeach
                      @endif
                    </select>
                    <div class="valid-feedback">Good job!</div>
